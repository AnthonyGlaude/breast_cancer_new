...
def init_worker(fasta_path: str, exon_parquet_path: str):
    ...
    for exon in global_gtf_exons.itertuples():
        ...
        start = exon.start
        end = exon.end + 1  # intervaltree half-open [start, end)
...
def _process_transcript_worker(transcript_id, mutations):
    ...
    for combo in all_subsets(mutations):
        ...
        if any(m['position'] not in mapping for m in combo):
            continue
        ms = _apply_mutations_to_transcript(transcript_id, seq, mapping, combo)
        sig = "_".join(f"{m['ref_nucleotide']}({m['alt_nucleotide']})" for m in combo)
        rid = f"{transcript_id}_mut_({sig})"
        rec = SeqRecord(Seq(ms), id=rid, description=global_genome[transcript_id].description)
        out.append(rec)
    return out
