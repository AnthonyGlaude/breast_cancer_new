...
def find_exons_for_mutations(vcf_df, exon_dict):
    exons = []
    for _, mutation in vcf_df.iterrows():
        found_exon = False
        for transcript_id, exons_info in exon_dict.items():
            for exon in exons_info:
                if mutation['chromosome'] == exon['chromosome'] and exon['start'] <= mutation['position'] <= exon['end'] and mutation['gene_name'] == mutation['gene_name']:
                    exons.append({
    ...
    for _, exon in exons_df.iterrows():
        ...
        if chromosome_key in genome:
            ...
            if start_index < 0 or end_index > len(seq_record.seq):
                print(f"Warning: Indices out of range for chromosome {chromosome} at position {position}.")
                continue
            ...
            if 0 <= ref_sequence_start < len(genomic_sequence) and ref_sequence_end <= len(genomic_sequence):
                ...
                if ref_sequence == exon['ref_nucleotide']:
                    ...
                    print(f"Warning: Reference nucleotide mismatch at {position}. Found: {ref_sequence}, Expected: {exon['ref_nucleotide']}")
            else:
                print(f"Warning: Indices out of range for chromosome {chromosome} at position {position}.")
        else:
...
def main():
    ...
    annotated_exons_df = find_exons_for_mutations(vcf_df, exon_dict)
